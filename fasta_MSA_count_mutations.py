...
for j in range(cols):
    try:
        idx_nochange = list(result[j].index).index('0')
        str_change = matrix_temp[0][j]+str((j+1))+matrix_temp[0][j]
        l_changes.append(str_change)
        l_qtd.append(result[j][idx_nochange])
        l_row_changes.append('-')
    except:
        print('Column:',j,' did not have 0 value')
    
    for i in range(1,rows):
        if (matrix_temp[i][j] != 'X') and (matrix_temp[i][j] != '0'):
